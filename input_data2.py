...
def load(inputs_dir, queue_capacity, img_size, channel=3, name='', mode='train', paired=True):
    with tf.device('/cpu:0'):
        ...
        if paired:
            image.set_shape([img_size, 3*img_size, channel])
        else:
...
def preprocess(image, img_size=256, channel=3, flip=False, crop_size=None, random_crop=False, paired=True, seed=None):
    with tf.device('/cpu:0'):
        if paired:
            ...
            imgA, imgB, imgC = tf.split(image, 3, axis=1)
            # crop
            if seed == None:
...
def batch_inputs(input_dir, batch_size, img_size=256, name='', channelA = 3, channelB = 3, channelC = 3, mode='train', flip=False, crop_size=None, random_crop=False):
    if mode == 'test':
        flip = False
    ...
    with tf.device('/cpu:0'):
        ...
        # preprocessing
        imgA, imgB, imgC = preprocess(image, img_size, channel, flip, crop_size, random_crop)
        if channel == 3:
            if channelA == 1:
        ...
        # make batch
        if mode == 'train':
            batchA, batchB, batchC = tf.train.shuffle_batch([imgA, imgB, imgC], batch_size=batch_size, num_threads = 4, capacity=16*batch_size, min_after_dequeue=8*batch_size,  name=name+'_batch_queue')
            return batchA, batchB, batchC, num_data
        elif mode == 'test':
            batchA, batchB, batchC, name_batch = tf.train.batch([imgA, imgB, imgC, filename], batch_size=batch_size, num_threads = 1, capacity=2*batch_size, name=name+'_batch_queue')
            return batchA, batchB, batchC, name_batch, num_data
        else:
